...
const POSTCODE_REGEX = new RegExp(/^(GIR|[A-Za-z]\d[A-Za-z\d]??|[A-Za-z]{2}\d[A-Za-z\d]??)[ ]??(\d[A-Za-z]{2})$/);
const UKNINO_REGEX = new RegExp(/^[A-CEGHJ-PR-TW-Z]{1}[A-CEGHJ-NPR-TW-Z]{1}[0-9]{6}[A-DFM]{0,1}$/);
const PHONE_REGEX = new RegExp(/^\d{10,11}$/);
const EMAIL_REGEX = new RegExp(/^[a-zA-Z0-9]+(\.[_a-zA-Z0-9]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,15})$/);
const VALID_CHARS_REGEX = new RegExp(/^([a-zA-Z()\[/\]0-9 &,'.-])*$/);
const NIC_REGEX = new RegExp(/^([a-zA-Z0-9 '.-/])*$/);
...
export { GUESTS_REGEX, DATE_REGEX, TIME_REGEX, stringsToDate, TITLE_REGEX, NAME_REGEX, POSTCODE_REGEX, UKNINO_REGEX, PHONE_REGEX, EMAIL_REGEX, VALID_CHARS_REGEX, ALPHANUMERIC_REGEX, NUMERIC_REGEX, NIC_REGEX };
